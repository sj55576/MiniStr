...
/** Produces the exact registered-scenario state that the game uses at turn one. */
export function createEditorGameState(state: ScenarioEditorState): GameResult<{ scenario: ScenarioDefinition; state: GameState }> {
  const validated = validateEditorScenario(state);
  if (!validated.ok) return validated;
  const scenario = validated.value[0]!;
  return { ok: true, value: { scenario, state: createScenarioInitialState(scenario) } };
}
...
function scenarioToData(scenario: { id: string; name: string; briefing: string; startingGold: number; board: { width: number; height: number; terrain: { kind: TerrainKind; owner?: PlayerId }[][] }; initialUnits: readonly InitialUnit[]; victoryConditions: readonly VictoryCondition[]; defeatConditions: readonly VictoryCondition[]; turnLimit?: number }): ScenarioData {
  return scenarioDefinitionToData(scenario as ScenarioDefinition);
}
