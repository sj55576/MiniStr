...
function render(): void {
  ...
  const forecasts = selectedUnit && selectedUnit.owner === game.activePlayer
    ? visibleEnemies(game, selectedUnit.owner).map(enemy => ({ enemy, forecast: forecastCombat(game, selectedUnit, enemy) })).filter((item): item is { enemy: typeof item.enemy; forecast: Extract<typeof item.forecast, { ok: true }> } => item.forecast.ok)
    : [];
  ...
  document.querySelector<HTMLButtonElement>('#end')!.onclick = () => { if (dispatch({ type: 'endTurn' })) { selected = undefined; undoStack = []; if (game.activePlayer === 'blue') runCpu(); } render(); };
