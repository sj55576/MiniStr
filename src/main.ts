...
const BOARD_MIN_TILE_SIZE = 40;
const BOARD_ZOOM_LEVELS = [1, 1.25, 1.5] as const;
...
function render(): void {
  ...
  const boardZoom = BOARD_ZOOM_LEVELS[boardZoomIndex]!;
  const boardTileSize = BOARD_MIN_TILE_SIZE * boardZoom;
  const boardViewportHeight = renderedGame.board.height * boardTileSize + 10;
  ...
  app.querySelector<HTMLButtonElement>('#skip-cpu')?.addEventListener('click', () => {
    ...
    message = 'CPU の残りの行動を高速に進めています。';
    render();
  });
  app.querySelector<HTMLButtonElement>('#board-zoom-out')?.addEventListener('click', () => {
    if (boardZoomIndex === 0) return;
    boardZoomIndex -= 1;
    focusSelector = `.tile[data-x="${focusedPosition.x}"][data-y="${focusedPosition.y}"]`;
    render();
  });
  app.querySelector<HTMLButtonElement>('#board-zoom-in')?.addEventListener('click', () => {
    if (boardZoomIndex === BOARD_ZOOM_LEVELS.length - 1) return;
    boardZoomIndex += 1;
    focusSelector = `.tile[data-x="${focusedPosition.x}"][data-y="${focusedPosition.y}"]`;
    render();
  });
...
function runCpu(): void {
  ...
  const advance = (): void => {
    ...
    const before = game;
    if (!dispatch(action)) { message = 'CPU の行動を安全に終了しました。'; finishCpuTurn(); return; }
